package com.example.auditlog.unit;
...
class HashChainServiceTest {
    ...
    void verifyChain_validChain_returnsTrue() {
        ...
        String hash1 = hashChainService.computeHash("GENESIS", id1, ts1, "u:1", "login", "app:1", Outcome.SUCCESS, null);
        ...
        AuditEventEntity e1 = AuditEventEntity.builder()
                .id(id1).timestamp(ts1).actor("u:1").action("login")
                .resource("app:1").outcome(Outcome.SUCCESS)
                .prevHash(null).eventHash(hash1).build();

        AuditEventEntity e2 = AuditEventEntity.builder()
                .id(id2).timestamp(ts2).actor("u:2").action("logout")
                .resource("app:1").outcome(Outcome.SUCCESS)
                .prevHash(hash1).eventHash(hash2).build();
    ...
    void verifyChain_tamperedEvent_returnsFalse() {
        ...
        AuditEventEntity tampered = AuditEventEntity.builder()
                .id(id1).timestamp(ts1).actor("u:1").action("login")
                .resource("app:1").outcome(Outcome.SUCCESS)
                .prevHash(null).eventHash("deadbeef").build();
